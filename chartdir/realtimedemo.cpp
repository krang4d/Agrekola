    ...
    QDialog(parent)
{
    ...
    // Delta Value display
    (new QLabel("Delta", frame))->setGeometry(6, 269, 48, 21);
    m_ValueD = new QLabel(frame);
    m_ValueD->setGeometry(55, 269, 59, 21);
    m_ValueD->setFrameShape(QFrame::StyledPanel);
    ...
    for (int i = 0; i < sampleSize; ++i)
        m_timeStamps[i] = m_dataSeriesA[i] = m_dataSeriesB[i] = m_dataSeriesC[i] = m_dataSeriesD[i] = Chart::NoValue;
...
//
void RealtimeDemo::getData(std::list<double> *dataList)
{
    // The current time
    ...
    do
    {
        ...
        double dataA = *it;//20 + cos(p * 129241) * 10 + 1 / (cos(p) * cos(p) + 0.01);
        double dataB = *(++it);//150 + 100 * sin(p / 27.7) * sin(p / 10.1);
        double dataC = *(++it);//150 + 100 * cos(p / 6.7) * cos(p / 11.9);
        double dataD = *(++it);//150 + 100 * tan(p / 1000) * tan(p / 1000);
        ...
        shiftData(m_dataSeriesD, sampleSize, dataD);
    ...
    m_ValueD->setText(QString::number(m_dataSeriesD[sampleSize - 1], 'f', 2));
}
...
void RealtimeDemo::drawChart()
{
    ...
    XYChart *c = new XYChart(780, 290, 0xf4f4f4, 0x000000, 1);
    ...
    c->setPlotArea(55, 62, 700, 195, 0xffffff, -1, -1, 0xcccccc, 0xcccccc);
    c->setClipping();
    ...
    if (lastTime != Chart::NoValue)
    {
        ...
        sprintf(buffer, "Services: <*bgColor=CCCCFF*> %.2f ", m_dataSeriesD[sampleSize - 1]);
        layer->addDataSet(DoubleArray(m_dataSeriesD, sampleSize), 0x0000ff, buffer);
    }
